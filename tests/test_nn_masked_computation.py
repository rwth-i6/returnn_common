    ...
    class _Net(nn.Module):
        ...
        def __call__(self, x: nn.Tensor, *, axis: nn.Dim) -> nn.Tensor:
            ...
            loop.state.lstm = self.lstm.default_initial_state(
                batch_dims=x.batch_dims_ordered(remove=(axis, x.feature_dim))
            )
            with loop:
                x_ = loop.unstack(x)
                mask = nn.reduce(x_, mode="mean", axis=x_.feature_dim) >= 0.0  # [B]
                with nn.MaskedComputation(mask):
                    loop.state.lstm_out, loop.state.lstm = self.lstm(
                        x_, state=loop.state.lstm, spatial_dim=nn.single_step_dim
                    )
