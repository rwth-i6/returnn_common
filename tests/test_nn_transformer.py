...
def test_nn_transformer_search():
    ...
    _, _, out_labels, _ = transformer(
        data, source_spatial_axis=time_dim, target=nn.SearchFunc(beam_size=3, max_seq_len=nn.dim_value(time_dim))
    )
    ...
    dec_self_att_layer_dict = net_dict["loop"]["unit"]["decoder"]["subnetwork"]["layers.0"]["subnetwork"]["self_attn"][
        "subnetwork"
    ]["k_accum"]
