...
def pprint(obj: Any, *, file=sys.stdout,
           prefix="", postfix="",
  ...
  def _sub_pprint(obj_: Any, prefix_="", postfix_="", inc_indent=True):
    ...
    if not multi_line and postfix_.endswith(","):
      postfix_ += " "
    pprint(
      obj_, file=file, prefix=prefix_, postfix=postfix_,
...
def _type_simplicity_score(obj: Any, _offset=0.) -> float:
  """
  :param Any obj:
  :param float _offset:
  ...
  if isinstance(obj, (int, numpy.integer)):
    if obj == 0:
      return 1. + _offset
    return 1. + numpy.log10(abs(obj)) + _offset
  if isinstance(obj, str):
    return 2. + len(obj) + _offset
  ...
  if isinstance(obj, numpy.ndarray):
    ...
    if obj.size * 2. + _offset > _type_simplicity_limit:  # too big already?
      return obj.size * 2. + _offset
