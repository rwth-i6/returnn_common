...
def py_print_iteration_info(msg, var, n, debug=True):
    ...
    if not debug:
        return var
    var_print = tf.print("n=", n, "\t", msg, tf.shape(var), var, summarize=-1, output_stream=sys.stdout)
...
def backtrack_alignment_tf(bt_ta, input_lengths, label_lengths, blank_index):
    ...
    def body(s, alignments, idx):
        ...
        align_write = tf.where(
            ...
            tf.ones((n_batch,), dtype=tf.int32) * blank_index,
        )  # outside, assume blank
        alignments = alignments.write(s, align_write)
        return s - 1, alignments, idx

    init_s = max_path - 1
    final_s, final_alignments_ta, final_idx = tf.while_loop(
        lambda s, *args: tf.greater_equal(s, 1), body, (init_s, alignments_ta, initial_idx)
    )
...
def tf_shift_logprobs(mat, axis):
    ...
    def fn(args):  # x: (B, U, V)
        ...
        x, shift = args
        padded = tf.pad(x, [[0, 0], [shift, max_time - shift], [0, 0]], constant_values=0)  # B  # U+T+1  # V
        return padded, shift
...
def tf_forward_shifted_rnnt(
    log_probs, labels, input_lengths=None, label_lengths=None, blank_index=0, debug=False, with_alignment=False
):
    """
    ...
    def body_forward(n, alpha_ta, *args):
        ...
        batchs, rows = tf.meshgrid(tf.range(n_batch), tf.range(labels_max_len), indexing="ij")
        ...
        cut_off = max_target
        alpha_y = tf.cond(tf.greater(n, max_target), lambda: alpha_y[:, :cut_off], lambda: alpha_y)
        lp_blank = tf.cond(tf.greater(n, max_target), lambda: lp_blank[:, :cut_off], lambda: lp_blank)
        alpha_blank = tf.cond(tf.greater(n, max_target), lambda: alpha_blank[:, :cut_off], lambda: alpha_blank)
        ...
        if with_alignment:
            ...
            b, r = tf.meshgrid(tf.range(n_batch), tf.maximum(0, tf.range(max_len_diag) - 1), indexing="ij")
            ...
            best_sel = tf.where(
                tf.tile(tf.equal(argmax_idx, 0)[..., None], [1, 1, 2]), stack_blank_sel, stack_emit_sel  # blank  # emit
            )
    ...
    if with_alignment:
        ...
        final_n, alpha_out_ta = tf.while_loop(
            cond, body_forward, [init_n, init_alpha_ta], parallel_iterations=1, name="rnnt"  # iterative computation
        )
    ...
    final_i, a_ta = tf.while_loop(lambda i, _: tf.less(i, n_batch), ta_read_body, (tf.constant(0, tf.int32), res_ta))
    indices = tf.stack(
        [
            tf.range(n_batch),
