...
class DecoderSlowRnnLstmIndependent(IDecoderSlowRnn):
    ...
    def __init__(self, num_layers=1, lstm_dim=512, zoneout=True, embed_dim=256, embed_dropout=0.2, **kwargs):
        ...
        self._lstm_opts = dict(
            num_layers=num_layers, lstm_dim=lstm_dim, zoneout=zoneout, embed_dim=embed_dim, embed_dropout=embed_dropout
        )
...
class DecoderFastRnnOnlyReadout(IDecoderFastRnn):
    ...
    def make(self, *, prev_label_wb: Tensor, encoder: Tensor, slow_rnn: Tensor) -> LayerDict:
        ...
        return {
            ...
            "subnetwork": {
                "in": LinearCombine(
                    dim=self.readout_dim, dropout=self.readout_dropout, l2=self.readout_l2, ctx=self.ctx
                ).make(
                    {
        ...
        return {
            ...
            "subnetwork": {
                ...
                "label_emit_log_prob": {
                    "class": "combine",
                    "kind": "add",
                    "from": [_base(log_prob_nb), "emit_log_prob"],
                },
...
class Decoder(_IMaker):
    ...
    def make(self, encoder: Tensor):
        ...
        rec_decoder = {
            ...
            "fast_rnn": self.fast_rnn.make(prev_label_wb="prev:output_", slow_rnn="slow_rnn", encoder="am"),
    ...
    return {
        ...
        "subnetwork": {
            "output_wo_b0": {
                ...
                "unit": {"class": "copy"},
                "from": "data",
                "mask": f"base:{decoder}/output_emit",
            },
            "output": {"class": "reinterpret_data", "from": "output_wo_b0", "set_sparse_dim": target.get_num_classes()},
        },
    }


def make_masked(
    source: str, *, mask: str, masked_from: Optional[str] = None, unmask: bool = True, unit: Dict[str, Any]
) -> Dict[str, Any]:
    """
    Make masked
    """
