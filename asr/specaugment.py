    ...
    *,
    ...
    with nn.Cond(nn.train_flag() | (not only_on_train)) as cond:
        ...
        x_masked = random_mask_v2(
            ...
            max_num=nn.minimum(
                nn.maximum(spatial_len // num_spatial_mask_factor, 2) * (1 + step1 + step2 * 2), spatial_len
            ),
    ...
    if isinstance(num, int):
        for i in range(num):
            x = _mask_v2(
                x,
                mask_axis=mask_axis,
                pos=nn.gather(indices, axis=k_dim, position=i),
                max_amount=max_dims,
...
def specaugment_v1(x: nn.Tensor, **kwargs) -> nn.Tensor:
    ...
    return nn.make_layer(
        {"class": "eval", "from": x, "eval": specaugment_v1_eval_func, "eval_locals": kwargs}, name="specaugment"
    )
