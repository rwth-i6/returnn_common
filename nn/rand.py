...
class Random(nn.Module):
    ...
    def __call__(self, **kwargs) -> nn.Tensor:
        ...
        return nn.random(explicit_state=state_var, auto_update_state=True, **kwargs)
    ...
    def normal(
        ...
        dtype: str = nn.NotSpecified,
        *,
        mean: Union[int, float, nn.Tensor] = 0.0,
        stddev: Union[int, float, nn.Tensor] = 1.0,
    ) -> nn.Tensor:
        """normal"""
    ...
    def bernoulli(
        self, shape: Sequence[nn.Dim], dtype: str = nn.NotSpecified, *, p: Union[float, nn.Tensor]
    ) -> nn.Tensor:
        """bernoulli. 0 or 1. p: prob for 1. uniform(1) <= p -> 1, else 0"""
        x = self.uniform(shape=shape, dtype=dtype, minval=0.0, maxval=1.0)
...
def random_bernoulli(shape: Sequence[nn.Dim], dtype: str = nn.NotSpecified, *, p: Union[float, nn.Tensor]) -> nn.Tensor:
...
def random(
    *,
    ...
    return nn.make_layer({"class": "random", **args}, name=name or "random")
