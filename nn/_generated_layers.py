...
RETURNN: 1.20230310.144740+git.124a6d2
...
def slice(
    ...
    *,
    ...
    if out_dim is None or out_dim is NotSpecified:
        out_dim = nn.Dim(kind=axis.kind, description=f"{_name_str(name, 'slice')}:out_dim", dimension=None)
    args = {
        "axis": axis,
    ...
    *,
    ...
    if out_spatial_dim is None or out_spatial_dim is NotSpecified:
        out_spatial_dim = nn.Dim(
            kind=axis.kind, description=f"{_name_str(name, 'slice_nd')}:out_spatial_dim", dimension=None
        )
    args = {
        "size": size,
        "start": start,
...
def random_state_init(
    *,
    ...
    :param int|Sequence[int]|numpy.ndarray|None seed:
...
def rec_window(
    ...
    *,
    ...
    if out_spatial_dim is None or out_spatial_dim is NotSpecified:
        out_spatial_dim = nn.Dim(
            kind=axis.kind, description=f"{_name_str(name, 'rec_window')}:out_spatial_dim", dimension=None
        )
    args = {
...
def merge_dims(
    ...
    *,
    ...
    if out_dim is None or out_dim is NotSpecified:
        ...
        out_dim = nn.Dim(kind=kind, description=f"{_name_str(name, 'merge_dims')}:out_dim", dimension=None)
    args = {
        "axes": axes,
        "out_dim": out_dim,
    }
...
def repeat(
    ...
    *,
    ...
    if out_dim is None or out_dim is NotSpecified:
        out_dim = nn.Dim(kind=axis.kind, description=f"{_name_str(name, 'repeat')}:out_dim", dimension=None)
    args = {
...
def prefix_in_time(
    ...
    *,
    ...
    if out_dim is None or out_dim is NotSpecified:
        out_dim = nn.Dim(kind=axis.kind, description=f"{_name_str(name, 'prefix_in_time')}:out_dim", dimension=None)
    args = {
        "axis": axis,
        "out_dim": out_dim,
        "prefix": prefix,
...
def postfix_in_time(
    ...
    *,
    ...
    if out_dim is None or out_dim is NotSpecified:
        out_dim = nn.Dim(kind=axis.kind, description=f"{_name_str(name, 'postfix_in_time')}:out_dim", dimension=None)
    args = {
        "axis": axis,
        "out_dim": out_dim,
        "postfix": postfix,
...
def time_chunking(
    ...
    *,
    ...
    if out_dim is None or out_dim is NotSpecified:
        out_dim = nn.Dim(kind=axis.kind, description=f"{_name_str(name, 'time_chunking')}:out_dim", dimension=None)
    args = {
...
def resize(
    ...
    *,
    ...
    if out_dim is None or out_dim is NotSpecified:
        out_dim = nn.Dim(kind=axis.kind, description=f"{_name_str(name, 'resize')}:out_dim", dimension=None)
    args = {
        "factor": factor,
        "axis": axis,
        "out_dim": out_dim,
...
def remove(
    ...
    *,
    ...
    if out_dim is None or out_dim is NotSpecified:
        out_dim = nn.Dim(kind=axis.kind, description=f"{_name_str(name, 'remove')}:out_dim", dimension=None)
    args = {
        "symbol": symbol,
        "axis": axis,
        "out_dim": out_dim,
...
def positional_encoding(
    ...
    *,
    ...
    :param None|nn.Tensor offset: Specify the offset to be added to positions.
...
def edit_distance_table(
    ...
    *,
    ...
    if out_dim is None or out_dim is NotSpecified:
        out_dim = nn.Dim(
            kind=axis.kind, description=f"{_name_str(name, 'edit_distance_table')}:out_dim", dimension=None
        )
    args = {
        "axis": axis,
        "debug": debug,
...
def rec_cum_concat(
    ...
    *,
    ...
    if out_spatial_dim is None or out_spatial_dim is NotSpecified:
        out_spatial_dim = nn.Dim(
            kind=axis.kind, description=f"{_name_str(name, 'rec_cum_concat')}:out_spatial_dim", dimension=None
        )
    args = {
        "out_spatial_dim": out_spatial_dim,
        "axis": axis,
...
def stft(
    ...
    *,
    frame_shift: int,
    frame_size: int,
    fft_size: Optional[int] = NotSpecified,
    in_spatial_dims: Optional[Sequence[nn.Dim]] = NotSpecified,
    ...
    :param Sequence[nn.Dim]|None in_spatial_dims:
    ...
    """
    if not isinstance(source, nn.Tensor):
        raise TypeError(f"stft: unexpected type for source {source!r}, need tensor")
    if out_spatial_dims is None or out_spatial_dims is NotSpecified:
        out_spatial_dims = [
            nn.Dim(kind=d.kind, description=f"{_name_str(name, 'stft')}:out_spatial_dims{i}", dimension=None)
    ...
    args = {key: value for (key, value) in args.items() if value is not NotSpecified}
    layer = nn.make_layer({"class": "stft", "from": source, **args}, name=name or "stft")
    return layer, out_spatial_dims
