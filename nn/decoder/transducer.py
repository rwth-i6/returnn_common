...
class ITransducerDecoderFullSum(ITransducerDecoder, ABC):
    ...
    def full_sum_label_seq_score(
        self, *, encoder: nn.Tensor, encoder_spatial_dim: nn.Dim, labels: nn.Tensor, labels_spatial_dim: nn.Dim
    ) -> nn.Tensor:
        """
...
def search_transducer(
    decoder: ITransducerDecoder, *, encoder: nn.Tensor, encoder_spatial_dim: nn.Dim
) -> (nn.Tensor, nn.Tensor):
    """
