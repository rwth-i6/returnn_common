...
from .base import (
    LabelTopology,
    TDecoderLabelSync,
    TDecoderJointNetLogProb,
    IDecoderLabelSyncStateScaledLogProbs,
    IDecoderLabelSyncAlignDepRnn,
    IDecoderJointBaseLogProb,
    IDecoderLabelSyncRnn,
    IDecoderLabelSyncLabelsOnlyRnn,
...
class Decoder(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.predictor = (
            predictor  # earlier: fast_rnn + readout. align-sync or matrix time * label. predicts align label
        )
    ...
    def __call__(
        ...
        *,
        ...
        with loop:
            ...
            encoder_frame = None
            if isinstance(  # only get it when we really need it
                self.label_predict_enc, IDecoderLabelSyncAlignDepRnn
            ) or isinstance(self.predictor, IDecoderJointBaseLogProb):
                ...
                with comp_context_mgr:
                    if isinstance(self.label_predict_enc, IDecoderLabelSyncRnn):
                        label_predict_enc, loop.state.label_predict_enc = self.label_predict_enc(
                            prev_label=loop.state.label_nb, encoder_seq=encoder, state=loop.state.label_predict_enc
                        )
                    elif isinstance(self.label_predict_enc, IDecoderLabelSyncLabelsOnlyRnn):
                        label_predict_enc, loop.state.label_predict_enc = self.label_predict_enc(
                            prev_label=loop.state.label_nb, state=loop.state.label_predict_enc
                        )
            ...
            if isinstance(self.predictor, IDecoderLabelSyncLogits):
                ...
                if isinstance(self.predictor, IDecoderJointNoStateLogProb):
                    ...
                    predictor_out, loop.state.predictor = self.predictor(
                        time_sync_in=encoder_frame, prev_align_label=loop.state.label_wb, state=loop.state.predictor
                    )
                else:
            ...
            if encoder_frame_idx is not None:
                if LabelTopology.is_time_sync(self.label_topology):
                    ...
                    loop.state.encoder_frame_idx = encoder_frame_idx + nn.cast(
                        align_label == self.target_blank_symbol, dtype="int32"
                    )
                else:
