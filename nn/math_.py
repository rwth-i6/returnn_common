...
def identity(x: nn.TensorRef) -> nn.TensorRef:
  """
...
def exp(x: nn.TensorRef) -> nn.Tensor:
  """exp. see also :func:`safe_exp`"""
  return _activation(x, activation="exp")


def safe_exp(x: nn.TensorRef, *, eps: float = 1e-7) -> nn.Tensor:
  """
...
def log(x: nn.TensorRef) -> nn.Tensor:
  """log. see also :func:`safe_log`"""
  return _activation(x, activation="log")


def safe_log(x: nn.TensorRef, *, eps: float = 1e-7, use_fake_grad: bool = True) -> nn.Tensor:
  """
...
def tanh(x: nn.TensorRef) -> nn.Tensor:
  """tanh"""
  return _activation(x, activation="tanh")


def sigmoid(x: nn.TensorRef) -> nn.Tensor:
  """sigmoid"""
  return _activation(x, activation="sigmoid")


def log_sigmoid(x: nn.TensorRef) -> nn.Tensor:
  """log sigmoid"""
  return _activation(x, activation="log_sigmoid")


def sqrt(x: nn.TensorRef) -> nn.Tensor:
  """sqrt"""
  return _activation(x, activation="sqrt")


def rsqrt(x: nn.TensorRef) -> nn.Tensor:
  """rsqrt"""
  return _activation(x, activation="rsqrt")


def swish(x: nn.TensorRef) -> nn.Tensor:
  """swish"""
  return _activation(x, activation="swish")


def squared_difference(a: nn.TensorRef, b: nn.TensorRef, *, name: Optional[str] = None) -> nn.Tensor:
...
def log_softmax(x: nn.TensorRef, *, axis: nn.Dim, **kwargs) -> nn.Tensor:
  """
...
def _activation(x: nn.TensorRef, activation: str) -> nn.Tensor:
  """
