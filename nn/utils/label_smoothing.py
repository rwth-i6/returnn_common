...
def label_smoothing(prob: nn.Tensor, smoothing: Union[nn.Tensor, float], *, axis: Optional[nn.Dim] = None) -> nn.Tensor:
    """
...
def label_smoothed_log_prob_gradient(
    log_prob: nn.Tensor,
    smoothing: Union[nn.Tensor, float],
    *,
    ...
    if exclude_labels:
        ...
        factor = nn.where(mask, factor, 1.0)
        floor_prob = nn.where(mask, floor_prob, 0.0)
