...
class _Rec(nn.Module):
    ...
    def __init__(
        self, in_dim: nn.Dim, out_dim: nn.Dim, *, unit: Optional[str] = None, unit_opts: Optional[Dict[str, Any]] = None
    ):
        """
...
class ZoneoutLSTM(_Rec):
    ...
    def __init__(
        self, in_dim: nn.Dim, out_dim: nn.Dim, *, zoneout_factor_cell: float = 0.0, zoneout_factor_output: float = 0.0
    ):
        super().__init__(
            in_dim=in_dim,
            out_dim=out_dim,
